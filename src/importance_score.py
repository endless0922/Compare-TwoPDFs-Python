...
def calculate_importance_score(suspicious_pairs):
    datadir = utils.get_models_directory()
    try:
    ...
    corpus = []
    dummies = []
    # Alpha order: (Common digit sequence, Common text string, Duplicate page, Identical image)
    ...
    if suspicious_pairs:
        ...
        importance_pred = utils.logistic(importance_pred)
    else:
        importance_pred = []
