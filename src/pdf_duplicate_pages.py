...
def find_duplicate_pages(data_a, data_b):
    ...
    m = pairwise_distances(x_, x_, metric="cosine")

    np.fill_diagonal(m, 1)
    m[np.tril_indices(len(m))] = 1

    page_tups_a = [
        (page_num, data_a["file_index"], text)
        for page_num, text in zip(page_nums_a, texts_a)
    ]
    page_tups_b = [
        (page_num, data_b["file_index"], text)
        for page_num, text in zip(page_nums_b, texts_b)
    ]
    combined_pages = page_tups_a + page_tups_b
    idxs_1, idxs_2 = np.where(m < 0.01)

    pairs = [
        ((combined_pages[i], combined_pages[j]), m[i][j])
...
def remove_duplicate_pages(file_data, duplicate_pages):
    """
    Rewrite the data object as if the duplicate pages didn't exist.
    ...
    new_image_hashes = [
        i for i in file_data["image_hashes"] if i[2] not in pages_to_ignore
    ]
